    ...
    // Normalizar / inicializar variables de sesión
    ...
    if ($action === 'activate_alumna') {
        ...
        header("Location: alumnas-maestras.php"); 
        exit;
    }
    ...
    if ($action === 'delete_maestra') {
        ...
        $redir = "alumnas-maestras.php";

        if (isset($_GET['alumnas'])) {
            $redir .= "?alumnas=" . intval($_GET['alumnas']);
        } elseif (isset($_GET['maestras'])) {
            $redir .= "?maestras=" . intval($_GET['maestras']);
        } elseif (isset($_GET['clientes'])) {
            $redir .= "?clientes=" . intval($_GET['clientes']);
        }
    ...
    // -------------------- LISTADOS --------------------
    // Detectar parámetros GET para mostrar activos o inactivos por tabla
    ...
    $sqlAlumnas  = "SELECT id_alumna, nombre, apat, amat, telefono, correo, direccion, descuento_aplicado, tipo_descuento, estatus
                    FROM alumnas 
                    WHERE estatus = $mostrar_alumnas
                    ORDER BY id_alumna ASC";
                    
    $sqlMaestras = "SELECT id_maestra, nombre, base, acuerdo, gastos, porcentaje_ganancia, estatus
                    FROM maestras 
                    WHERE estatus = $mostrar_maestras 
                    ORDER BY id_maestra ASC";

    $sqlClientes = "SELECT * FROM clientes 
                    WHERE estatus = $mostrar_clientes 
